...
export class Renderer {
  ...
  render(mapData, camera, player, options = {}) {
    ...
    if (options.debug) this._drawNpcSpawns(mapData);
  ...
  _drawNpcSpawns(mapData) {
    const ctx = this.ctx;
    const ts = this.tileSize;
    for (const spot of mapData.npcSpawns) {
      const cx = spot.x * ts + ts / 2;
      const cy = spot.y * ts + ts / 2;
      ...
      ctx.textAlign = 'center';
      ctx.fillText('NPC予定地', cx, cy - ts * 0.5);
    }
  }
