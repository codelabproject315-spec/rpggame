...
/**
 * lines は通常は文字列の配列だが、おみくじのように毎回内容を
 * 変えたい場合は (state, npc) => string[] という関数でもよい。
 * ここで両方のケースを吸収し、常に配列を返す。
 */
function resolveLines(lines, state, npc) {
  const resolved = typeof lines === 'function' ? lines(state, npc) : lines;
  return resolved && resolved.length ? resolved : [''];
}
...
export class DialogueManager {
  ...
  startNpcDialogue(npc, state) {
    ...
    return {
      speaker: npc.name,
      lines: resolveLines(rule.lines, state, npc),
  ...
  resolveChoice(choice, npc, state) {
    ...
    return {
      speaker: npc.name,
      lines: resolveLines(choice.lines, state, npc),
      choices: null,
    };
  }
