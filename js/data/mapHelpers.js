 ...
 *
 * visibleWhen: 常には出現させたくないNPC（隠しNPCなど）向けのオプション。
 * 'night' を指定すると、夜の間だけ姿を現す（それ以外は移動もすり抜けも自由）。
 * 省略すれば今まで通り常に表示・当たり判定ありになる。
 */
export function placeNpc(grid, npcs, { id, type, x, y, name, facing = 'down', note = '', visibleWhen = null }) {
  // 常時表示のNPCだけ、足元を通行不可にする（隠しNPCは普段そこに"いない"ので通行の妨げにしない）
  if (!visibleWhen) {
    fillRect(grid, x, y, 1, 1, TileType.OBJECT_FOOTPRINT);
  }
  npcs.push({ id, type, x, y, name, facing, note, visibleWhen, onInteract: null });
}
