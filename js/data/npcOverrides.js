...
export const NPC_OVERRIDE_DIALOGUES = {
      ...
      choices: [
        {
          ...
          lines: [
            ...
            'よろしければ、この御守りをお持ちください。',
          ],
          effect: (s) => { s.setFlag('legendRevealed'); s.setFlag('legendCurious'); s.collectItem('item_charm_shrine'); },
        },
        {
          text: '静かにうなずく',
          lines: ['……多くは語りますまい。', 'ただ、この町とあなたの間には、何か縁があるのかもしれませんね。', 'この御守りを、お守り代わりにどうぞ。'],
          effect: (s) => { s.setFlag('legendRevealed'); s.setFlag('legendRespectful'); s.collectItem('item_charm_shrine'); },
        },
      ],
    },
    {
  ...
  forest_mystery_01: [
    ...
    },
    {
      ...
      choices: [
        {
          text: '「話してくれてありがとう」',
          lines: ['……礼を言われるとはな。', 'これからも、この森を頼むぞ。', '……これを持っていけ。森番の証だ。'],
          effect: (s) => { s.setFlag('mysterySecretRevealed'); s.setFlag('mysteryGrateful'); s.collectItem('item_charm_forest'); },
        },
        {
          text: '黙って頷く',
          lines: ['……お前らしいな。', '言葉はいらぬ、ということか。', '……これを受け取れ。多くは語らんが、それでいい。'],
          effect: (s) => { s.setFlag('mysterySecretRevealed'); s.setFlag('mysteryQuiet'); s.collectItem('item_charm_forest'); },
        },
      ],
    },
    {
  ...
  // ---- 森: 木こり（のこぎりをくれる） ----
  forest_woodcutter_01: [
    {
      when: (s) => s.hasFlag('hasSaw'),
      lines: ['そののこぎり、大事に使えよ。', '町のあちこちの茂みも、それなら切り開けるはずだ。'],
    },
    {
      lines: [
        'おう、こんなところまで来たのか。',
        '町のあちこちの生垣、茂りすぎて困ってる家も多いだろう。',
        'これを持っていきな、のこぎりだ。',
      ],
      effect: (s) => s.setFlag('hasSaw'),
    },
  ],
      ...
      choices: [
        {
          ...
          lines: ['……ありがとう。', 'いつかまた、ゆっくり話しますね。', 'よかったら、これを……星の形をしたペンダントなんです。'],
          effect: (s) => { s.setFlag('stargazerSecretRevealed'); s.setFlag('stargazerListened'); s.collectItem('item_charm_star'); },
        },
        {
          text: '何も言わず、隣に座る',
          lines: ['……何も言わないでいてくれるんですね。', 'それだけで、救われる気がします。', '……これ、受け取ってもらえますか。'],
          effect: (s) => { s.setFlag('stargazerSecretRevealed'); s.setFlag('stargazerComforted'); s.collectItem('item_charm_star'); },
        },
      ],
    },
    {
  ...
  residential_mystery_01: [
    ...
    },
    {
      ...
      choices: [
        {
          text: '「それは嬉しいです」',
          lines: ['……そう言ってもらえると、決めてよかったと思えるよ。', '……これを渡しておくよ。今の気持ちを書いた手紙だ。'],
          effect: (s) => { s.setFlag('rainMysterySecretRevealed'); s.setFlag('rainMysteryGrateful'); s.collectItem('item_charm_rain'); },
        },
        {
          text: '黙って微笑む',
          lines: ['……その微笑みだけで、十分だよ。', '……これを、受け取ってくれないか。'],
          effect: (s) => { s.setFlag('rainMysterySecretRevealed'); s.setFlag('rainMysteryQuiet'); s.collectItem('item_charm_rain'); },
        },
      ],
    },
    {
